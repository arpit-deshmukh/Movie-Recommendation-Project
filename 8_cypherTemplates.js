...
function validateStep(step) {
  switch (step.type) {
    ...
    case "sort": {
      const [sLabel] = step.field.split(".");
      if (!ALLOWED_LABELS.has(sLabel)) throw new Error(`Invalid label: ${sLabel}`);
      if (!["ASC", "DESC"].includes(step.direction?.toUpperCase()))
...
// Build Cypher from validated plan
    ...
    switch (step.type) {
      ...
      case "aggregation": {
        ...
        if (step.groupBy) {
          ...
          returnClause =
            `RETURN ${grpVar}.${grpProp}, ` +
            `${step.function}(${aggTarget}) AS ${alias}`;
        } else {
          const [aggLabel] = (step.field || "").split(".");
          const aggTarget = LABEL_VAR_MAP[aggLabel] || "*";

          returnClause =
            `RETURN ${step.function}(${aggTarget}) AS ${alias}`;
        }
        break;
      }
